print("Hello World")
print("I just wrote my first Python Program")
print("This is so much fun")

""" This code will

Prompt for your name

set the "name' variable and print a message."""

name = input("Hello, what is your name? ")  #gets the user's name

print("Welcome, it is so nice to meet you " + name) #prints a message with the user's name

#The following assignment will print to the screen: "It is raining cats and dogs." including the appropriate spacing.

c = "cats"
d = "dogs"
s = "It is raining " + c + " and " + d + "."
print(s)
